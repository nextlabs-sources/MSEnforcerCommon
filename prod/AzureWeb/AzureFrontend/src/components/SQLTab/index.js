...
const SQLTab = ({
    uid, type, name, server, handler, tip = '', isActive = false, paddingLeft = '0',
}) => {
    const tabWrapper = {
        height: '40px',
        lineHeight: '40px',
    ...
    const defaultHandler = (e) => {
        e.stopPropagation()
        handler && handler(server, name)
    }
    const getPath = path2Reg.compile(config.Routes.tables)

    return (
        <div style={tabWrapper} onClick={defaultHandler} title={tip} role='menuitem'>
            <NavLink to={`${getPath({ uid, id: name, type, })}`} isActive={() => true} activeStyle={{
                display: 'block',
                overflow: 'hidden',
