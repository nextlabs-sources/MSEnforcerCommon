...
export default class Token {
  ...
  public async get() {
    ...
    const { token_type, access_token, expires_in, access_date } = JSON.parse(window.sessionStorage.getItem('NextLabs_ControlCenter_Token')) || {};
  ...
  private storeToken(token_type: string, access_token: string, expires_in: string) {
    window.sessionStorage.setItem('NextLabs_ControlCenter_Token', JSON.stringify({
      token_type,
      access_token,
      expires_in,
