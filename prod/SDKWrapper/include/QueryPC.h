...
public:
    CRequest():m_srcAttres(NULL), m_dstAttres(NULL),m_userAttres(NULL),m_AppAttres(NULL),m_lPerformOB(1), m_lNoiseLevel(2), m_attrsLogObligation(NULL)
    {
    }
...
private:
	...
	/**Log Obligation Information*/
	wstring m_wstrLogIdentifier;
	wstring m_wstrObligationName;
	CComPtr<ICEAttres> m_attrsLogObligation;
...
public:
    ...
    */
    STDMETHOD(set_action)(BSTR ceAction);
    ...
    */
    ...
    /**
    *  @brief Set Log obligation parameters
    *
    *  @param bstrLogIdentifier  [IN] log identifier.
    *  @param bstrObligationName [IN] olbigation name.
    *  @param pAttres [IN] log attribute.
    *  @retval S_OK success otherwise failed.
    */
	STDMETHOD(set_logObligationInfo)(BSTR bstrLogIdentifier, BSTR bstrObligationName, ICEAttres* pAttres);
...
{
public:
    CQueryPC():m_dwCookie(0),m_hHandle(NULL),m_ipNumber(0)
    {
    }
...
private:
    ...
    */
    ...
    LONG m_ipNumber;                    /**< ip number from client*/
...
public:
   /** 
    *  Set Log Obligation data    
    *  @param lCookie  [IN] the cookie number which get by the method: get_cookie().
	*  @param plResult [IN] the result that pc return
    *  @retval S_OK success otherwise failed.
    */

	STDMETHOD (log_obligationdata)(LONG lCookie, LONG* plResult);	
    ...
    *  @Set ip number.
    *
    *  @param ipNumber   [IN] ip number pass to control conter.
    *  @retval S_OK success otherwise failed.
    */
    STDMETHOD(set_ip_number)(LONG ipNumber);    
    /**
    *  @}
    */
};
