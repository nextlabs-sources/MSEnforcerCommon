...
bool CNLTagManager::GetTagDllHandle()
{
    ...
    if (NULL == m_hLib)
    {
         NLPrintLogW(g_kbOutputDebugLog,L"Load Library [%s] Failed! The error code is [%d]", wstrLib.c_str(), GetLastError());
        return false;
    }
    ...
    if (NULL == m_hMgr)
    {
		NLPrintLogW(g_kbOutputDebugLog,L"Load Library [%s] Failed! The error code is [%d]", wstrMgr.c_str(), GetLastError());
        return false;
    }
...
bool CNLTagManager::GetTagFunAddr()
{
    ...
    {
		NLPrintLogW(g_kbOutputDebugLog,L"failed to get resattrlib/resattrmgr functions\r\n");
        return false;
    }
    return true;
}
