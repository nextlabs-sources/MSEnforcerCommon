...
STDMETHODIMP CRequest::set_logObligationInfo(BSTR bstrLogIdentifier, BSTR bstrObligationName, ICEAttres* pAttres)
{
	m_wstrLogIdentifier = bstrLogIdentifier;
	m_wstrObligationName = bstrObligationName;
	m_attrsLogObligation = pAttres;
	return S_OK;
}
...
STDMETHODIMP CQueryPC::set_ip_number(LONG ipNumber)
{
    m_ipNumber = ipNumber;
    return S_OK;
}
...
{NLONLY_DEBUG
    ...
    if (!m_sdk.is_loaded())
    {
		...
		strCommonPath += L"bin32";
            ...
            if (m_sdk.load(strCommonPath.c_str()))
            {
                ...
                CEResult_t ret = m_sdk.fns.CECONN_Initialize(ceApp, ceUser, NULL, &m_hHandle, 1000);
...
STDMETHODIMP CQueryPC::check_resourceex_json(BSTR strRequest, BSTR *strResponse, LONG* plResult)
{
	...
	for (int i = 0; i <= (total - 1) / MAX_NUM_REQUESTS; ++i)
	{
		...
		{					
			NLPRINT_DEBUGVIEWLOG(L"CEEVALUATE_CheckResourcesEx failed, result=%d", ceResult);
...
STDMETHODIMP CQueryPC::log_obligationdata(LONG lCookie, LONG* plResult)
{  
    if (!InitSDK())
    {
        NLPRINT_DEBUGVIEWLOG(L"init SDK failed \n");
    ...
    { 
        ...
        if (pReq != NULL)
		{
			 CEString cestrLogId    = m_sdk.fns.CEM_AllocateString(pReq->m_wstrLogIdentifier.c_str());
			 CEString cestrObligationName    = m_sdk.fns.CEM_AllocateString(pReq->m_wstrObligationName.c_str());
			 CEAttributes ceAttLogObligation = NLGetAttributes(pReq->m_attrsLogObligation);

			 *plResult = m_sdk.fns.CELOGGING_LogObligationData(m_hHandle, cestrLogId, cestrObligationName, &ceAttLogObligation);
			 
			 
			 NLFreeCEString(&cestrLogId);
			 NLFreeCEString(&cestrObligationName);
 			 NLFreeAttributes(&ceAttLogObligation);
...
{NLONLY_DEBUG
    ...
    CEint32 ceIpNum = (CEint32)m_ipNumber;
...
CEUser CQueryPC::NLAllocCEUserInfo(_In_ const std::wstring& wstrUserName, _In_ const std::wstring& wstrUserID)
{
    ...
    //for pc8.0 pc can get user sid , we did not need pass this to pc
