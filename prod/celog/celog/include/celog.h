  ...
  public:
     ...
     */
	 ...
	 nchars = (*it)->Log(sline);
      ...
      int Log(int lvl, const char* file, int line, const char* fmt, va_list argptr) throw()
      {
          assert(fmt != NULL);
          ...
          */
          ...
          {
              timeline[strlen(timeline) - 1] = (char)NULL;  /* remove newline */
              p += _snprintf_s(p, _countof(sline), _TRUNCATE, "%.19hs.%03hu %hs: ", timeline, timebuffer.millitm, &timeline[20]);
          }
          ...
          if (file != NULL)
          {
              p += _snprintf_s(p, _countof(sline) - (p - sline), _TRUNCATE, "%hs [%d]: ", file, line);
          }
          ...
          //terminating null. If the entire string (with terminating null) fits in buffer, then these functions return the number of 
          //characters written (not including the terminating null); otherwise, these functions return -1 to indicate that truncation occurred.
          int nchars = _vsnprintf_s(p, nbufsize, nbufsize - 1, fmt, argptr);
