...
AstExpr *parse_value(Lex *lex, ParseException *e) {
    if (lex->GetCurrent()->GetType() == Token::TK_STR_LITERAL) {
        ...
        r->SetValue(array);
        return r;
    }else if (lex->GetCurrent()->GetType() == Token::TK_CALL_FUNCTION) {
        lex->Next();
        ...
        lex->Next();
        AstConstantValue *r = new AstConstantValue(AstExpr::C_CALL_FUNCTION);
        r->SetValue(array);
        return r;
    } else {
        ...
        if (e->_syms == NULL) { val_type = AstColumnRef::CC_OTHER;
            ...
            switch (col_type) {
                ...
                case AstColumnRef::RES: {
                    assert(ids.size() == 2);
                    val_type = to_val_type(e->_syms->GetAttrTypeByPmnameAttrName(ids[0]->GetId(), ids.back()->GetId()));
                } break;
