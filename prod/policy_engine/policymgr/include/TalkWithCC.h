...
class TalkWithCC202004 : public TalkWithCC {
public:
	TalkWithCC202004(NXLHttpClient *http_client, const std::string& user, const std::string& pwd);
	...
	virtual bool CasSecurityCheck(http::response<http::string_body>& res) override;
protected:
	virtual bool GetRequestResult(std::string& out, http::request<http::string_body> & req) override;

	std::string access_token; // e.g. AT-10-yEZyMs-FL0aodqsKDereRZ5jnhSo0mkr
	std::string id_token; // e.g. eyJhbGciOiJSU0EtT0FFUC0yNTYiLCJlbmMiOiJBMjU2R0NNIiwia2l
	std::string refresh_token; // e.g. RT-10-6-QO99z9fzLaPOR0yUrHZdW-jk5FFrCa
	std::string token_type; // i.e. bearer
	int expires_in; // 1200 seconds
	// std::string scope;
	std::string cookie;
};
#endif